...
app.post('/api/admin/jobs', requireAuth, requireLeader, async (req: Request, res: Response) => {
    ...
    try {
        const targetUser = await db.get<User>('SELECT id, role FROM Users WHERE id = ?', [assigned_to_user_id]);
        if (!targetUser) {
 ...
 */
app.post('/api/jobs/:id/submit', requireAuth, upload.single('proofImage'), async (req: Request, res: Response) => {
...
app.post('/api/users', requireAuth, requireChiefLead, async (req: Request, res: Response) => {
    ...
    try {
        if (role === 'chief_lead') {
            const countRow = await db.get<{ count: number }>("SELECT COUNT(*) as count FROM Users WHERE role = 'chief_lead'");
            if (countRow && countRow.count >= 3) return res.status(403).json({ error: 'Maximum limit of 3 chief leads reached.' });
        }
 ...
 */
app.post('/api/teams/:id/members', requireAuth, requireChiefLead, async (req: Request, res: Response) => {
    const { user_id, role_in_team } = req.body;
    try {
        await db.run('INSERT INTO Team_Members (team_id, user_id, role_in_team) VALUES (?, ?, ?)', [
            req.params.id,
            user_id,
            role_in_team,
        ]);
...
app.post('/api/leader/teams/:id/members', requireAuth, async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (targetUser.role !== 'member')
            return res.status(403).json({ error: 'Leaders can only add members to teams, not other leaders.' });

        await db.run("INSERT INTO Team_Members (team_id, user_id, role_in_team) VALUES (?, ?, 'member')", [
            req.params.id,
            user_id,
        ]);
...
app.post('/api/admin/teams/:id/events', requireAuth, async (req: Request, res: Response) => {
    ...
    try {
        ...
        await db.run('INSERT INTO Team_Events (team_id, title, start_date, end_date) VALUES (?, ?, ?, ?)', [
            req.params.id,
            title,
            start_date,
            end_date,
...
app.get('/api/teams/my-teams', requireAuth, async (req: Request, res: Response) => {
    try {
        ...
        for (const team of teams) {
            ...
            team.events = await db.all<TeamEvent[]>('SELECT * FROM Team_Events WHERE team_id = ?', [team.id]);
            // Explicitly cast the result of db.all to an array of TeamMemberWithUser
            const members = (await db.all(
                `SELECT u.id, u.username, tm.role_in_team, u.role
                 FROM Users u
...
app.get('/api/members-list', requireAuth, async (_req: Request, res: Response) => {
    try {
        const members = await db.all<User[]>("SELECT id, username, role FROM Users WHERE role = 'member'");
        res.json({ members });
    } catch (err) {
